...
class Group_Model extends CI_Model
{
  ...
  public function create($groupName)
  {
    ...
    if ($query->num_rows() > 0) {
      ...
      // Enroll adm in this group
      $data = array(
        "myuser_id" => 1,
        "mycourse_id" => 1,
        "mymodule_id" => 1,
        "mylesson_id" => 1,
        "mygroup_id" => $this->db->insert_id(),
        "question_id" => 1,
        "answer_id" => 1,
        "program_id" => 1
      );
  ...
  /*
    ...
    ==============================================
    */
  public function getStudentsInsideGroup($groupId, $email = '')
  {
    $email = $this->Filters_Model->filterStudentsByEmail($email);

    $this->db->select("T1.id,T1.name, T1.avatar, T1.email");
    $this->db->distinct();
    $this->db->from("relationship T0");
