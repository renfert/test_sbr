            ...
            <ul class="list-group">
                <draggable v-model="sections" ghost-class="ghost" @end="finishRepositioning" > 
                    <transition-group type="transition" name="flip-list">
                        <li v-for="element in sections" :key="element.id" class="list-group-item d-flex justify-content-between align-items-center sortable">
                            <!-- Banner -->
                            <span class="text-sabiorealm" v-if="element.banner_id != 1">
                                Banner
                            </span>
                            <!-- Product list -->
                            <span class="text-sabiorealm" v-if="element.product_list_id != 1">
                                Product list 
                            </span>
                            <!-- Testimonial list -->
                            <span class="text-sabiorealm" v-if="element.testimonial_id != 1">
                                Testimonial
                            </span>
                            <!-- Text and media list -->
                            <span class="text-sabiorealm" v-if="element.text_and_media_id != 1">
                                Text and media
                            </span>
                            
                            <div class="action-icons">
                                <!-- Edit banner -->
                                <i v-if="element.banner_id != 1" @click.prevent="openEditSectionModal(element.id,'banner')" class="el-icon-edit text-sabiorealm links"></i>

                                <!-- Edit product list -->
                                <i v-if="element.product_list_id != 1" @click.prevent="openEditSectionModal(element.id,'product-list')" class="el-icon-edit text-sabiorealm links"></i>

    
                                <!-- Edit testimonial -->
                                <i v-if="element.testimonial_id != 1" @click.prevent="openEditSectionModal(element.id,'testimonial')" class="el-icon-edit text-sabiorealm links"></i>

                                <!-- Edit text and media list -->
                                <i v-if="element.text_and_media_id != 1" @click.prevent="openEditSectionModal(element.id,'text-and-media')" class="el-icon-edit text-sabiorealm links"></i>
...
export default {
    ...
    methods: {
        ...
        deleteSection: function(id){
            ...
            var urlToBeUsedInTheRequest = this.getUrlToMakeRequest("section", "delete");
        ...
        reorderLinkPositions: function(){
            ...
            var urlToBeUsedInTheRequest = this.getUrlToMakeRequest("section", "reorder");
            axios.post(urlToBeUsedInTheRequest, formData).then(() => {
                /* Success callback */
                 this.updateSectionListArray();
