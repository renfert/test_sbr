<template>
  <div
    ref="content"
    class="content-page"
    @touchmove="detectBottom($event)"
    @scroll="detectBottom($event)"
  >
    <el-card
      shadow="always"
      style="background-color: #009cd8; color: white; text-align: center"
      class="h1"
    >
      {{ course_name }}
    </el-card>
...
export default {
  ...
  methods: {
    getCourseName() {
      ...
      this.$request
        .post(this.$getUrlToMakeRequest('course', 'get'), form)
        .then((response) => {
