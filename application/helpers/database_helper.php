...
function database_connect(){
  ...
  if(!isset($connection)) {
    $connection = mysqli_connect('localhost', 'root', '', 'eadtools_bit4learn');
  }
...
/**
 * Queries the database, returning an object or false, based on
 * the query's results. The query must have no arguments (i.e. useful for
 * retrieving all values from a table).
 * @param $query - The query to the database, as a string.
 * @return associative array or false
 */
function database_no_args_query_obj($query) {
  $connection = database_connect();
  $result = mysqli_query($connection, $query);
